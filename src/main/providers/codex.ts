...
async function hasAuth(): Promise<boolean> {
  try {
    const value = JSON.parse(await readFile(join(codexHome(), "auth.json"), "utf8")) as {
      auth_mode?: unknown;
      tokens?: { access_token?: unknown };
    };
    return typeof value.tokens?.access_token === "string" || typeof value.auth_mode === "string";
  } catch {
...
export function normalizeCodexRateLimits(raw: RateResponse): {
  ...
  const mapped = raw.rateLimitsByLimitId ? Object.entries(raw.rateLimitsByLimitId) : [];
  ...
  for (const [limitId, snapshot] of snapshots) {
    ...
    for (const [slot, value] of [
      ...
      const rawDuration = typeof value.windowDurationMins === "number" ? value.windowDurationMins : null;
      const duration = rawDuration !== null && Math.abs(rawDuration - 300) <= 2
        ? 300
...
export function parseCodexUsage(output: string, now: number): UsageWindow[] {
  ...
  const patterns: Array<[string, string, number, RegExp]> = [
    ["session", "5h", 300, /(?:5h|session)(?:\s+limit)?[^%]{0,100}?(\d+(?:\.\d+)?)%([^|·]{0,80})/i],
    ["weekly", "Weekly", 10_080, /(?:weekly|week|7-day)(?:\s+limit)?[^%]{0,100}?(\d+(?:\.\d+)?)%([^|·]{0,80})/i]
  ];
  return patterns.flatMap(([id, label, durationMinutes, pattern]) => {
    ...
    const usedPercent = /left|remaining|available/i.test(tail) ? 100 - raw : raw;
...
async function fetchViaRpc(
  ...
  return new Promise((resolve, reject) => {
    ...
    let timeout = setTimeout(() => finish(new ProviderError("timeout", "Codex app-server initialization timed out")), 30_000);
    ...
    child.on("error", () => finish(new ProviderError("rpc-unavailable", "Codex app-server failed to start")));
    child.on("exit", () => finish(new ProviderError("rpc-unavailable", "Codex app-server exited early")));
    child.stdout.on("data", (chunk: Buffer) => {
      ...
      for (let newline = buffer.indexOf("\n"); newline >= 0; newline = buffer.indexOf("\n")) {
        ...
        if (phase === "initialize" && message.id === 1) {
          if (message.error) return finish(new ProviderError("rpc-unavailable", "Codex RPC initialization failed"));
          phase = "limits";
          clearTimeout(timeout);
          timeout = setTimeout(() => finish(new ProviderError("timeout", "Codex rate-limit request timed out")), 10_000);
          send({ jsonrpc: "2.0", method: "initialized" });
          send({ jsonrpc: "2.0", id: 2, method: "account/rateLimits/read", params: null });
        } else if (phase === "limits" && message.id === 2) {
          if (message.error) return finish(new ProviderError("rpc-unavailable", "Codex rate limits are unavailable"));
...
export class CodexProvider implements UsageProvider {
  ...
  async fetchUsage(context: UsageFetchContext): Promise<ProviderUsageSnapshot> {
    ...
    if (!detection.executablePath) throw new ProviderError("not-installed", "Codex CLI not detected");
    ...
    try {
      ...
      if (!windows.length) throw new ProviderError("parse", "Codex returned no usage windows");
      return this.snapshot(windows, plan, context.now, "rpc");
    } catch (error) {
      if (context.signal.aborted) throw new ProviderError("aborted", "Cancelled");
      if (error instanceof ProviderError && error.kind === "not-authenticated") throw error;
    }
    ...
    const output = await runPty({
      ...
      complete: (value) => /(?:5h|weekly|7-day)[\s\S]*?\d+(?:\.\d+)?%/i.test(value)
    });
    const windows = parseCodexUsage(output, context.now);
    if (!windows.length) throw new ProviderError("parse", "Could not parse Codex usage");
