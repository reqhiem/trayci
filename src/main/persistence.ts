...
export function configDirectory(): string {
  return join(process.env.XDG_CONFIG_HOME ?? join(homedir(), ".config"), "trayci");
}
...
export class CacheRepository {
  ...
  async load(): Promise<Partial<Record<string, ProviderUsageSnapshot>>> {
    try {
      ...
      if (!object(value) || value.schemaVersion !== 1 || !object(value.providers)) return {};
      return Object.fromEntries(
        Object.entries(value.providers)
          .filter((entry): entry is [string, ProviderUsageSnapshot] => validSnapshot(entry[1]))
          .map(([id, snapshot]) => [id, { ...snapshot, status: "stale", source: "cache", error: null }])
      );
    } catch {
      return {};
    }
  ...
  async save(providers: Partial<Record<string, ProviderUsageSnapshot>>): Promise<void> {
    const safe = Object.fromEntries(
      Object.entries(providers)
        .filter((entry): entry is [string, ProviderUsageSnapshot] => Boolean(entry[1]))
        .map(([id, snapshot]) => [id, {
...
function validatePatch(patch: TrayciSettingsPatch): void {
  if (!object(patch) || Object.keys(patch).some((key) => !settingsKeys.has(key))) {
    throw new TypeError("Invalid settings patch");
  }
  if (patch.startOnLogin !== undefined && typeof patch.startOnLogin !== "boolean") {
    throw new TypeError("startOnLogin must be a boolean");
  }
  if (patch.refreshOnResume !== undefined && typeof patch.refreshOnResume !== "boolean") {
    throw new TypeError("refreshOnResume must be a boolean");
  }
  if (
    patch.pollIntervalMinutes !== undefined &&
    (!Number.isFinite(patch.pollIntervalMinutes) || patch.pollIntervalMinutes < 5 || patch.pollIntervalMinutes > 1440)
  ) {
    throw new TypeError("pollIntervalMinutes must be between 5 and 1440");
  }
  if (patch.displayMode !== undefined && !["detailed", "compact"].includes(patch.displayMode)) {
  ...
  if (patch.providers !== undefined) {
    if (!object(patch.providers) || Object.keys(patch.providers).some((key) => key !== "claude" && key !== "codex")) {
      throw new TypeError("Invalid provider settings");
    }
    for (const provider of Object.values(patch.providers)) {
      if (!object(provider) || Object.keys(provider).some((key) => key !== "enabled" && key !== "executablePath")) {
        throw new TypeError("Invalid provider settings");
      }
      ...
      if (
        ...
        (typeof provider.executablePath !== "string" || !isAbsolute(provider.executablePath))
      ) {
        throw new TypeError("Provider executablePath must be an absolute path or null");
      }
    }
  }
}
...
export class SettingsRepository {
  ...
  async load(): Promise<TrayciSettings> {
    try {
      const raw = JSON.parse(await readFile(this.path, "utf8")) as TrayciSettingsPatch & {
...
export class AutostartService {
  ...
  async setEnabled(enabled: boolean, executable: string, args: string[] = []): Promise<void> {
    if (!enabled) {
