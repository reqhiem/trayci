...
The Mint row was checked with the packaged Electron runtime, real Claude and Codex sessions, the renderer E2E, and XDG autostart writing in an isolated configuration directory. Antigravity's parser and credential-free cache path are covered by unit tests; a real Google OAuth session still needs packaged manual validation. GNOME still requires a compatible desktop session for manual validation.
