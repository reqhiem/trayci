...
- Drag the popover by its title bar to any spot on screen; on X11 the position sticks between sessions
...
The popover can be dragged on Wayland too, but only an X11 session remembers where it was left: GTK lets no Wayland client place its own window, so it reopens wherever the compositor puts it. Running `GDK_BACKEND=x11 trayci` on a Wayland session is **not** a workaround — under XWayland the popover never takes input focus and the first click dismisses it instead of reaching the button.
