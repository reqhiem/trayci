...
import type { ProviderUsageSnapshot, UsageProvider } from "../../src/shared/types";
...
it("preserves the last successful snapshot after a temporary failure", async () => {
  ...
  const provider: UsageProvider = {
    id: "claude",
    displayName: "Claude",
    detect: async () => ({ provider: "claude", status: "available", executablePath: "/fake" }),
    fetchUsage: async ({ now }): Promise<ProviderUsageSnapshot> => {
      ...
      return {
        ...
        status: "ok",
        plan: null,
        windows: [{ id: "session", label: "5h", usedPercent: 42, durationMinutes: 300, resetsAt: null, resetDescription: null }],
        updatedAt: now,
        source: "oauth",
        error: null
      };
